...
1. On first power-on (or after factory reset) the feeder starts a
   Wi-Fi access point: **PetFeeder-XXXX** (XXXX = last 4 of MAC).
...
4. On submit the feeder attempts to connect. On success it saves the
   config, reboots into normal mode, and publishes its online state
   to the broker.
