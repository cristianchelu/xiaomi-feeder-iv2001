...
void test_tare_cli_show_pending_unset_stable(void)
{
    ...
    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(1), "tare drift: 0 g"));
...
void test_tare_cli_show_anchored_state(void)
{
    tare_cli_test_reset();
    auto_tare_anchor(42);
    auto_tare_idle_sample(42, true);

    TEST_ASSERT_EQUAL_UINT8(0, tare_cli_run_show());

    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(2), "tare stable: 42 g"));
    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(1), "tare drift: 0 g"));
...
void test_tare_cli_show_reports_drift_offset(void)
{
    ...
    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(2), "tare stable: 12 g"));
    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(1), "tare drift: 1 g"));
    TEST_ASSERT_NOT_NULL(strstr(app_log_test_line_from_end(0), "tare pending: no"));
}
